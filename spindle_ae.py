...
def save_ex(vae_name, ds, n_eval, log_loc, tar_name='vae_eval.tar.gz'):
    ...
    if ds.shape > 2:
        vae = load_model(vae_name,
                         custom_objects={'latent_dim': latent_dim, 'epsln_std': epsln_std, 'cvae_loss': vae_loss})
    else:
        vae = load_model(vae_name,
                         custom_objects={'latent_dim': latent_dim, 'epsln_std': epsln_std, 'vae_loss': vae_loss})
...
def vae_vis(data_path):
    x = read_csv(data_path + 'orig.csv', 3, 113)
    x_p = read_csv(data_path + 'recon.csv', 3, 113)
    ...
    for i in range(n_eval):
        ...
        ax1.set_xticks([])
        ax1.set_yticks([])
        plt.xlim(0, x.shape[1], x.shape[2])
        plt.ylim(0, x.shape[1], x.shape[2])
...
def train_vae(rds, dataFile, ds_name, vae_name, n_epochs=10, val_split=0.2, conv=False):
    ...
    print('Fitting model...')
    vae.fit(ds, ds, epochs=n_epochs, verbose=1, validation_split=val_split, shuffle='batch',
